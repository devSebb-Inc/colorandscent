...
export function ImageGallery({ images, productName, badge, badgeColor, selectedColor }: Props) {
  ...
  const selectedColorImages = useMemo(() => {
    ...
    return images.filter((img) => img.variantColor === selectedColor)
  }, [images, selectedColor])

  const visibleImages = selectedColorImages.length > 0
    ? selectedColorImages
    : fallbackImage
      ? [fallbackImage]
      : []

  const defaultActiveImage = visibleImages[0] ?? null

  const [activeImageId, setActiveImageId] = useState<string | null>(defaultActiveImage?.id ?? null)

  useEffect(() => {
    setActiveImageId(defaultActiveImage?.id ?? null)
  }, [defaultActiveImage?.id])

  const activeImage = visibleImages.find((img) => img.id === activeImageId) ?? defaultActiveImage
