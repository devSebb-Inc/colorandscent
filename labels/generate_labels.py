#!/usr/bin/env python3
"""
Color & Scent — Candle Label Design v14 (Simple)
...
def draw_design(output_path, candle_name, scent_notes, seed=42):
    img = Image.new('RGBA', (CANVAS_W, CANVAS_H), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    # Border
    draw_border(draw, CANVAS_W, CANVAS_H, GOLD, opacity=0.30)

    # Brand name (top)
    brand_font = load_font(FONT_SANS, 11)
    center_text(draw, "COLOR&SCENT", 45, brand_font, (*GOLD, 230), CANVAS_W, letter_spacing=5)

    # Thin line under brand
    draw_thin_line(draw, CANVAS_W/2, 68, 130, (*GOLD, 150))

    # Main title
    title_font = load_font(FONT_SERIF, 78)
    center_text(draw, candle_name.upper(), 110, title_font, DARK_TEXT, CANVAS_W, letter_spacing=4)

    # Thin line under title
    draw_thin_line(draw, CANVAS_W/2, 210, 240, (*GOLD, 160))

    # Scent description
    scent_font = load_font(FONT_SANS, 14)
    center_text(draw, scent_notes, 238, scent_font, (*GOLD, 240), CANVAS_W, letter_spacing=3)

    # Product details
    detail_font = load_font(FONT_SANS, 11)
    center_text(draw, "SOY WAX · COTTON WICK · HAND-POURED", 278, detail_font, (*DARK_TEXT, 180), CANVAS_W, letter_spacing=3)

    # Thin line
    draw_thin_line(draw, CANVAS_W/2, 315, 180, (*GOLD, 120))

    # Bottom claims
    claims_font = load_font(FONT_SANS, 10)
    center_text(draw, "VEGAN · ECO-FRIENDLY", 338, claims_font, (*GOLD, 220), CANVAS_W, letter_spacing=5)

    # Decorative thin line at bottom
    draw_thin_line(draw, CANVAS_W/2, 368, 100, (*GOLD, 80))
...
if __name__ == '__main__':
    ...
    draw_design(f'{out_dir}/01-amber-glow.png', "AMBER GLOW", "WARM AMBER · SANDALWOOD · VANILLA", seed=42)
    draw_design(f'{out_dir}/02-after-hours-revised.png', "AFTER HOURS", "LEATHER · LABDANUM · CEDARWOOD", seed=99)
